...
#
# Adapted from pkdxtools/pkdx for the standalone nash-mbt repo. The vendored
# mizchi/* packages only land after `moon update`, so this script runs after
# update and before each `moon build` / `moon test`.
set -euo pipefail
